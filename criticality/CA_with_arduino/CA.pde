...
class CA {
  ...
  int generation;  // How many generations? Keeping track of generations
  int numbercells = 40;
  int[] ruleset;     // An array to store the ruleset, for example {0,1,1,0,1,1,0,1}

  int w = 20; //width of one cell
  ...
  void randomize() {
    for (int i = 0; i < 8; i++) {
      ruleset[i] = int(random(2)); // array with 8 
    }
  }
  
  void rulegenerator(byte rulenumber) {
    for (int i = 0; i < 8; i++, rulenumber <<= 1) {
     ruleset[i] = (rulenumber & 128) == 0 ? 0 : 1;   
    }
  }
  ...
  // The process of creating the new generation // function to compute the next generation
  void generate() {
